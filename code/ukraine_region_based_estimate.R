...
get_ukraine_region_based_rosa <- function(max_ratio = .3, write_file = T){
  cat("generating region based estimate for Ukraine \n")
  region_pop_ukraine <- read.csv("region_pop_ukraine.csv", as.is = T)
  dt <- read.csv("../data/aggregate/UA-aggregate.csv", as.is = T)
  ...
  dt2$iso.3166.2[dt2$iso.3166.2 == ""] <- "UA"
  ...
  # total population
  total_pop <- region_pop_ukraine$pop_district[1]
  A11 <- c()
  A12 <- c() 
  A21 <- c()
  A22 <- c()
  ...
  for (j in dates){
    ...
    # exclude all country
    regions <- unique(dt2$iso.3166.2)[unique(dt2$iso.3166.2) != "UA"]
    lreg <- length(regions)
    ...
    for (i in 1:lreg){
      ...
      pop_current <-  region_pop_ukraine$pop_district[region_pop_ukraine$iso31662 == reg]
  ...
  region_based_estimate2 <- data.frame(date = dates,
                                       ...
                                       stringsAsFactors = F)
  if(write_file == T){
    cat("writing the region based estimate for Ukraine..\n")
    write.csv(region_based_estimate2, paste0("../data/PlotData/UA_regional_estimates/region_based_estimates/",
                                             "UA-region-based-estimate_rosa.csv"))
  }
  else{
...
get_ukraine_region_based_rosa(write_file = T)