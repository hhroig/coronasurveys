spain_provincial_regional_estimate <- function(countrycode = "ES",
                                               province = T,
                                               district = F,
                                               W = 90,
                                               alpha = 0.0001,
  ...
  if(write_summary_file == T){
    cat(paste0("Writing the region based estimate for", countrycode, "..\n"))
...
brazil_provincial_regional_estimate <- function(province = F,
                                               district = F,
                                               W = 90,
                                               alpha = 0.0001,
                                               max_ratio = .3,
  ...
  dt <- dt[is.finite(dt$ratio), ]  # discard cases with zero reach
  dt <- dt[dt$ratio <= max_ratio, ]
  dt$iso.3166.2[dt$iso.3166.2 == ""] <- "BR"
  # removal of multiple responses using cookies
  ...
  for (j in dates){
    ...
    if (write_daily_file == T){
      dt_est_count <- data.frame(countrycode = unique(dtregs$countrycode),
                                 population_country = sum(dtregs$population_region),
                                 estprop_country = estprop_country,
                                 meanprop_country = meanprop_country, 
                                 sumreach_country = sumreach_country,
      ...
      dt_est_prov_reg_country <- merge(dt_est_prov_reg, dt_est_count, all = T, by = "countrycode")
      write.csv(x = dt_est_prov_reg_country, file = paste0("../data/PlotData/BR_regional_estimates/daily_province_region_country_based_estimates/",
                                                           "BR-province-region-country-based-estimate-", gsub("/", "_", j), ".csv"))
    }
  ...
  region_based_estimate <- data.frame(date = dates,
                                      ...
                                      meanprop_country_agg = meanprop_country_agg)
  if(write_summary_file == T){
    cat("Writing the region based estimate for Brazil..\n")
    write.csv(region_based_estimate, paste0("../data/PlotData/BR_regional_estimates/region_based_estimates/",
                                            "BR-province-region-country-based-estimate.csv"))
  }
  else{
    return(region_based_estimate)
  }
...
brazil_provincial_regional_estimate(write_summary_file = T, write_daily_file = T)